 ...
 * @api private
 */
function matchCore(value, expected, partialMode) {
  if (value === null) return expected === null;
  if (expected === null) return value === null;
  partialMode = partialMode | false;
  switch (type(expected)) {
    ...
    case 'object':
      return compareObjects(value, expected, partialMode); // eslint-disable-line no-use-before-define
    case 'array':
...
function compareArrays(array, expected, partialMode) {
  ...
  return expected.every((item, index) => matchCore(array[index], expected[index], partialMode));
}
...
function compareObjects(object, expected, partialMode) {
  if (!compareArrays(Object.keys(object).sort(), Object.keys(expected).sort(), partialMode)) return false;
  return Object.keys(expected).every(key => matchCore(object[key], expected[key], partialMode));
}
 ...
 * @api public
 */
module.exports.bake = (expected) => value => matchCore(value, expected, false);
 ...
 * @api public
 */
module.exports.partial.bake = (expected) => value => matchCore(value, expected, true)
